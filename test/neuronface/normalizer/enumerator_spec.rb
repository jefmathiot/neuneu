...
describe Neuronface::Normalizer::Enumerator do
  it "uses inner normalizers to convert inputs" do
    normalizers = [
      Minitest::Mock.new.expect(:convert, 3, [1]),
      Minitest::Mock.new.expect(:convert, 4, [2])
    ]
    _(Neuronface::Normalizer::Enumerator.new(normalizers).convert([1, 2])).must_equal [3, 4]
  end
  ...
  it "uses inner normalizers to revert outputs" do
    ...
    _(Neuronface::Normalizer::Enumerator.new(normalizers).revert([1, 2])).must_equal [3, 4]
  end
end
